"""
attack_02.py — PGD (Projected Gradient Descent) adversarial attack.

What this script does:
1. Loads the trained model
2. Loads 200 test images from CIFAR-10
3. Runs PGD at four epsilon values
...
ADV_DIR = EVIDENCE_ADV / "pgd"
LOG_DIR = EVIDENCE_LOGS / "pgd"
...
def save_evidence(results: dict, epsilon: float, n_save: int) -> None:
    ...
    # — those are the only ones where a flip is a genuine attack success
    indices = [
        i for i in range(len(labels))
        if clean_preds[i] == labels[i]
    ]
...
def main() -> None:
    ...
    plot_results(summary, "PGD", ADV_DIR)
